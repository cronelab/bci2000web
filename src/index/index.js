...
window.onload = () =>{
    ...
    bci.onconnect = e => {
        ...
        setInterval (() => {
            bci.execute( 'Get System State', result => result).then(x => {
                console.log(x)
                updateState(x.trim())
            })
        }, 1000)
...
const loadTask = async (taskName) => {
    var arr = [].slice.call(document.getElementById('task-card').children).map(x=>x.remove())
...
const updateState = (state) => {
    document.getElementById('state-label').innerHTML = `<strong>${state}</strong>`;
    ...
    if (stateClasses[state]) document.getElementById('state-label').classList.add(stateClasses[state])
}
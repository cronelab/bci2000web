...
const Footer = () => {
  ...
  const bciOperator = useStore.getState().bciOperator;
  // const {
  //   setNumItems,
  //   setMenuState,
  //   setKeyboardState,
  ...
  const popover = (
    <Popover id="stateSimulator">
      ...
      <Popover.Body>
        ...
        <Link href="/tasks" >
          <Button>Research mode</Button>
        </Link>
      </Popover.Body>
    </Popover>
  );

  const keyboardState = useStore.getState().keyboardState;
  const setKeyboardState = (arg: string) => useStore.setState({keyboardState: arg})
  const menuState = useStore.getState().menuState;
  const setMenuState = (arg: string) => useStore.setState({menuState: arg})
  return (
    <Navbar style={{ backgroundColor: '#1e90ff' }} expand="lg" fixed="bottom">
      ...
      >
        <Nav>
          ...
          <Button className="selectableItem" 
          // ref={favoriteRef}
          >
            <BookmarkButton />
          </Button>
